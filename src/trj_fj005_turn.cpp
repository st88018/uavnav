...
int main(int argc, char **argv)
{
  ...
  while(ros::ok()){
    ...
    if(mission_state==TURN2)
    {
      static generalMove gm(ros::Time::now().toSec(),
                            takeoff_x, takeoff_y, takeoff_z,  PI,
                            takeoff_x, takeoff_y, takeoff_z,  PI*2, 5);
      gm.getPose(ros::Time::now().toSec(),pose);
      if(gm.finished())
      {
    ...
    if(mission_state==TURN3)
    {
      static generalMove gm(ros::Time::now().toSec(),
                            takeoff_x,     takeoff_y,   takeoff_z, 2*PI,
                            takeoff_x, takeoff_y, takeoff_z,  PI, 5);
      gm.getPose(ros::Time::now().toSec(),pose);
      if(gm.finished())
      {
    ...
    if(mission_state==TURN4)
    {
      static generalMove gm(ros::Time::now().toSec(),
                            takeoff_x, takeoff_y, takeoff_z,  PI,
                            takeoff_x, takeoff_y, takeoff_z,  0, 5);
      gm.getPose(ros::Time::now().toSec(),pose);
      if(gm.finished())
      {
